		    ...
		    if(response.ticker.sell <= priceToBuy) //Se valor de venda menor ou igual ao preço definido para compra
		    {
				 getBalance(coin, response.ticker.sell, true, (balance) => {
					 ...
					 // // tradeApi.placeBuyOrder(qtyToBuy, response.ticker.sell, 
						 // // (data) => {
							 //log('Ordem de compra de ' + qtyToBuy + ' ' + coin + ',  por '  + response.ticker.sell + ', inserida no livro. >> ' + data)
							 ...
							 // // tradeApi.placeSellOrder(data.quantity, response.ticker.sell * parseFloat(process.env.PROFITABILITY), 
								 ...
								 // // (data) => log('Erro ao inserir ordem de venda  de ' + data.quantity + '  ' + coin + ',  por '  + response.ticker.sell + ' >> '  + data))
				...
				if( response.ticker.buy >= priceToSell || (response.ticker.buy >= (response.ticker.high * 0.99)) ) 
				{
					getBalance(coin, response.ticker.buy, false, (balance) => {
						 ...
						 if(balance > 0){
							...
							// (data) => {
							  // (data) => log('Ordem de venda de ' + qtyToSell + '  ' + coin + ',  por '  + response.ticker.last + ',  inserida no livro. ' + data),
						 	  // (data) => log('Erro ao inserir ordem de venda  de ' + qtyToSell + '  ' + coin + ',  por '  + response.ticker.last + ' >> '  + data))
							// }
...
//chama funcao trader
tradeCoin('BCH', '4800', '0000', true, true);
